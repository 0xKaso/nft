...
contract Main is ERC721 {
    ...
    function mintGuest(uint8 times, bool isWhite) external payable  {
        ...
        if (isWhite){
            require((_tokenIds.current() + times) <= supplyTotal && (userHasMint[msg.sender] + times) == 1, "ERR_MINT_OVERFLOW_MAX");
        }  else {
            require((_tokenIds.current() + times) <= supplyTotal && (userHasMint[msg.sender] + times) <= 100, "ERR_MINT_OVERFLOW_MAX");
